...
| 6 | MVP Hardening for Low-cost Host | In Progress | 2026-02-23 | - | Codex app-server auth/runtime wired for web + WhatsApp command flow (`/auth connect/status/limits/disconnect`) with OAuth-first chat turns and OpenAI API key fallback. Remaining: retention, backup reminders, CI, performance tuning. |
