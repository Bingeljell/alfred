...
| 6 | MVP Hardening for Low-cost Host | In Progress | 2026-02-23 | - | Codex app-server auth/runtime wired for web + WhatsApp command flow (`/auth connect/status/limits/disconnect`) with OAuth-first chat turns and OpenAI API key fallback. Added restart-safe stale-thread recovery, boot-time auth status visibility in web console, persisted auth telemetry (`lastLogin`/disconnect/status checks) across restarts, and richer UI in-flight/status ergonomics with newest-first console option. Remaining: retention, backup reminders, CI, performance tuning. |
