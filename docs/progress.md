...
| 6 | MVP Hardening for Low-cost Host | In Progress | 2026-02-23 | - | Codex app-server auth/runtime wired for web + WhatsApp command flow (`/auth connect/status/limits/disconnect`) with OAuth-first chat turns and OpenAI API key fallback. Added restart-safe stale-thread recovery, boot-time auth status visibility in web console, persisted auth telemetry (`lastLogin`/disconnect/status checks) across restarts, richer UI in-flight/status ergonomics with newest-first console option, and first real Baileys live runtime wiring (`/v1/whatsapp/live/status/connect/disconnect`) with inbound token protection/reconnect controls, QR-print onboarding, required-prefix + sender-allowlist gating, optional self-message mode for one-number testing, and a guided in-UI WhatsApp setup flow for easier onboarding. Remaining: retention, backup reminders, CI, performance tuning. |
